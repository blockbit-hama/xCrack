            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {data.profitability_metrics.profit_by_strategy.map((strategy, index) => (
                  <div key={strategy.strategy} className="bg-gray-50 dark:bg-gray-700 p-4 rounded">
                    ...
                    <p className="text-sm text-gray-500">{strategy.percentage.toFixed(1)}%</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {data.strategy_performance.map((strategy) => (
                    <tr key={strategy.strategy} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      <td className="px-4 py-4 font-medium">{strategy.strategy}</td>
                      <td className="px-4 py-4">{strategy.total_opportunities.toLocaleString()}</td>
                      <td className="px-4 py-4">{strategy.successful_trades.toLocaleString()}</td>
                      <td className="px-4 py-4">
                        <span className={`font-semibold ${
                          strategy.success_rate > 0.7 ? 'text-green-600' : 
                          strategy.success_rate > 0.4 ? 'text-yellow-600' : 'text-red-600'
                        }`}>
                      ...
                      <td className="px-4 py-4">{strategy.avg_analysis_time_ms}ms</td>
                      <td className="px-4 py-4">
                        <span className={`font-semibold ${
                          strategy.gas_efficiency > 0.8 ? 'text-green-600' : 
                          strategy.gas_efficiency > 0.6 ? 'text-yellow-600' : 'text-red-600'
                        }`}>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="text-center">
                ...
                <p className="text-2xl font-bold">{data.gas_analytics.avg_gas_price_gwei} Gwei</p>
              </div>
              <div className="text-center">
                <h3 className="text-sm font-medium text-gray-500">평균 가스 사용량</h3>
                <p className="text-2xl font-bold">{data.gas_analytics.avg_gas_used.toLocaleString()}</p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className={`text-2xl font-bold ${
                  data.gas_analytics.gas_efficiency_score > 0.8 ? 'text-green-600' : 
                  data.gas_analytics.gas_efficiency_score > 0.6 ? 'text-yellow-600' : 'text-red-600'
                }`}>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {data.gas_analytics.gas_usage_by_strategy.map((strategy) => (
                  <div key={strategy.strategy} className="bg-gray-50 dark:bg-gray-700 p-4 rounded">
                    <h4 className="font-medium">{strategy.strategy}</h4>
                    <p className="text-lg font-bold">{strategy.gas_used.toLocaleString()}</p>
                    <p className="text-sm text-gray-500">{strategy.percentage.toFixed(1)}%</p>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="text-center">
                ...
                <p className={`text-2xl font-bold ${
                  data.system_health.uptime_percentage > 99 ? 'text-green-600' : 
                  data.system_health.uptime_percentage > 95 ? 'text-yellow-600' : 'text-red-600'
                }`}>
                  {formatPercentage(data.system_health.uptime_percentage / 100)}
                </p>
              </div>
              <div className="text-center">
                <h3 className="text-sm font-medium text-gray-500">응답 시간</h3>
                <p className="text-2xl font-bold">{data.system_health.avg_response_time_ms}ms</p>
              </div>
              <div className="text-center">
                <h3 className="text-sm font-medium text-gray-500">에러율</h3>
                <p className={`text-2xl font-bold ${
                  data.system_health.error_rate < 0.01 ? 'text-green-600' : 
                  data.system_health.error_rate < 0.05 ? 'text-yellow-600' : 'text-red-600'
                }`}>
              ...
              <div className="text-center">
                ...
                <p className="text-2xl font-bold">{data.system_health.memory_usage_mb}MB</p>
              </div>
              <div className="text-center">
                <h3 className="text-sm font-medium text-gray-500">CPU</h3>
                <p className="text-2xl font-bold">{formatPercentage(data.system_health.cpu_usage_percentage / 100)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-blue-600">
                  {formatPercentage(data.competitive_analysis.market_share_percentage / 100)}
                </p>
              </div>
              <div className="text-center">
                <h3 className="text-sm font-medium text-gray-500">경쟁자 수</h3>
                <p className="text-2xl font-bold">{data.competitive_analysis.competitor_count}</p>
              </div>
              <div className="text-center">
