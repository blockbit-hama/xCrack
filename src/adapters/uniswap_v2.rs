...
impl UniswapV2Adapter {
    pub fn new(config: AdapterConfig) -> Self {
        Self {
            router_address: Address::from_slice(&[
                0x7a, 0x25, 0x0d, 0x56, 0x30, 0xb4, 0xcf, 0x53, 0x97, 0x39, 0xdf, 0x2c, 0x5d, 0xac, 0xb4, 0xc6, 0x59, 0xf2, 0x48, 0x8d
            ]),
            factory_address: Address::from_slice(&[
                0x5c, 0x69, 0xbe, 0xe7, 0x01, 0xef, 0x81, 0x4a, 0x2b, 0x6a, 0x3e, 0xd4, 0xaa, 0x1b, 0xb9, 0xd3, 0x1c, 0x1e, 0x1f, 0x1a
            ]),
            config,
    ...
    /// Uniswap V2 견적 계산 (getAmountsOut 시뮬레이션)
    async fn calculate_amounts_out(&self, amount_in: U256, path: &[Address]) -> Result<Vec<U256>> {
        ...
        for i in 0..path.len() - 1 {
            // Mock 계산: 실제로는 풀의 리저브를 조회해야 함
            let current_amount = amounts[i];
            let output_amount = current_amount * U256::from(995) / U256::from(1000); // 0.5% 수수료
    ...
    /// swapExactTokensForTokens calldata 생성
...
impl DexAdapter for UniswapV2Adapter {
    ...
    async fn quote(
        ...
        debug!("Uniswap V2 quote: {} -> {} = {}", amount_in, amount_out, response_time);
        ...
        Ok(Quote {
            ...
            amount_out_min,
            price_impact,
            gas_estimate: 150000, // Uniswap V2 일반적인 가스 사용량
            valid_for: 60, // 1분
